...
var initializeVersionControl = function () {
	var _ref = _asyncToGenerator(function* (target) {
		...
		if (git) {
			var gitignore = trimLeft(`
		node_modules
		/build
		/*.log
		`) + '\n';
			var gitignorePath = _path2.default.resolve(target, '.gitignore');
			...
			var gitUser = 'Preact CLI<developit@users.noreply.github.com>';
			yield (0, _crossSpawnPromise2.default)('git', ['commit', '--author', gitUser, '-m', 'initial commit from Preact CLI'], { cwd });
		}
	});
...
exports.default = (0, _asyncCommand2.default)({
	...
	builder: {
		...
		type: {
			...
			choices: ['default', 'root', 'simple', 'empty'],
			default: 'default'
		},
		less: {
	...
	handler(argv) {
		return _asyncToGenerator(function* () {
			...
			pkg.scripts = _extends({}, pkg.scripts || {}, {
			...
			if (argv.install) {
				...
				yield npm(target, ['install', '--save-dev', 'preact-cli', 'if-env', 'eslint', 'eslint-config-synacor'].concat(argv.sass ? ['node-sass', 'sass-loader'] : [], argv.less ? ['less', 'less-loader'] : []).filter(Boolean));
...
var trimLeft = function trimLeft(string) {
